...
define(function(require){
    ...
    var Widget = ViewComponent.extend({
		...
		doActive: function(event){
			//lzg 很难受啊,需要记录第一次的触发，当内部创建完成后派发
			if(!this._modelActiveEvent) this._modelActiveEvent = $.extend({},event,{source:this});
		...
		load: function(fn){
			...
			if(re.test(this.url)){
				...
				var compose = this.compose = new WindowContainer({
					...
					onLoad: function(){
						fn && fn();
						if(me._modelActiveEvent)
							me.fireEvent('onActive',me._modelActiveEvent);
					},
