define(function(require) {
	...
	var paramValueHasEditor = ['db','tableName','permissions'];
	var Model = function() {
		this.callParent();
	...
	var params2xml = function($action, params) {
		if ($.isArray(params)) {
			$.each(params, function(i, param) {
				$action.append(justep.String.format('<{0} name="{1}" type="{2}"></{0}>', param.kind, param.name, param.type));
				if(param.value && param.name){
					var $p = $action.children("[name="+param.name+"]");
					//$p.attr("value",param.value);
					$p.append(param2xml(param));
				}
			});
		}
	};
