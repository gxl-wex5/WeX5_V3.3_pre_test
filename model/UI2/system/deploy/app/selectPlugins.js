define(function(require) {
	...
	Model.prototype.modelLoad = function(event) {
		...
		$.each(pluginsInfo, function() {
			...
			addPluginConfig(this.id, "common", this.commonParams);
			addPluginConfig(this.id, "android", this.androidParams);
			addPluginConfig(this.id, "ios", this.iosParams);
			this.hasParams = (this.commonParams && (this.commonParams.length > 0)) || (this.androidParams && (this.androidParams.length > 0)) || (this.iosParams && (this.iosParams.length > 0));
		});
		...
		if (this.getParent().edit) {
			...
			pluginsData.getRowByID('org.apache.cordova.splashscreen').val('checked', true);
		}
