define(function(require){
	...
	Model.prototype.userDataTablesCellRender = function(event){
		if(event.colName == 'uOperate'){
			event.html = "<button class='btn btn-link btnHint'>详情</button><button class='btn btn-link btnUpdate'>修改</button><button class='btn btn-link btnDelete'>删除</button>";
		}
	};
	...
	Model.prototype.userDataTablesRowClick = function(event){
	    ...
	    //单选checkbox，选中当前行，但是上一次选中的行才会变色，为什么？？？
	...
	Model.prototype.queryButtonClick = function(event){
		...
		temp.each(function(options){//遍历data
			if(name != options.row.val("uName")){//判断name是否相同
	...
	Model.prototype.jsButtonClick = function(event){
		util.showSource({
			self: this,
			data: 'demo/sample/table/tableTemplate/tableTemplate.js'
		});
	};
	...
	Model.prototype.buttonClick = function(event){
		util.showSource({
			data: 'demo/sample/table/tableTemplate/tableTemplate.w&xid=window',
			self: this,
			type: true
		});
	};
