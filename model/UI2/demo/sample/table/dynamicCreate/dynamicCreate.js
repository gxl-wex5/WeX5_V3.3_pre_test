...
//	    var parentNode = this.getElementsByXid('tableDiv');//切记，这个是根据xid获取Element列表
	   ...
	   newTable.setCSS({"margin": "10px"});//设置CSS样式
	};
	...
	Model.prototype.showJSButtonClick = function(event){
		util.showSource({
			data: 'demo/sample/table/dynamicCreate/dynamicCreate.js',
			self: this
		});
	};
	...
	Model.prototype.showSourceBtnClick = function(event){
		util.showSource({
			data: 'demo/sample/table/dynamicCreate/dynamicCreate.w&xid=window',
			self: this,
			type: true
		});
	};
