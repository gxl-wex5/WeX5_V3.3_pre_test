...
function autoStore(appPref){
	if(localStorage && localStorage.getItem("cordova_native_pref")){
		try{
			var nativePrefValue = localStorage.getItem("cordova_native_pref");
			var parsedValue = JSON.parse(nativePrefValue);
			for(var key in parsedValue){
				appPref.store(key,parsedValue[key]);
			}
		}catch(e){
		}
	}
